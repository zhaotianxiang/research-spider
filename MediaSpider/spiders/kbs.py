...
def generateUrlList(dateStr):
    return 'https://news.kbs.co.kr/api/getNewsList?currentPageNo=1&rowsPerPage=500&exceptPhotoYn=Y&broadCode=0001&broadDate={}&needReporterInfo=Y&orderBy=broadDate_desc%2CbroadOrder_asc'.format(
        dateStr)


class KBS(scrapy.Spider):
    name = 'kbs'
    start_urls = list(map(generateUrlList, dateRange("20210101", "20220304")))
