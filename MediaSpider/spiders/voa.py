...
class Voa(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in links:
            ...
            yield scrapy.Request(url=link.url,meta={"detail_url": link.url,"list_url": response.request.url},callback=self.parse_voa_detail)
    ...
    def parse_voa_detail(self,response):
        ...
        if links:
            for link in links:
                self.logger.info("VOA PARSE LIST ------------------- {} ".format(link.url))
                yield scrapy.Request(url=link.url,meta={"title":title,"content":content, "reporter_url":link.url, "detail_url":meta["detail_url"], "list_url":meta["list_url"]},callback=self.parse_voa_reporter)
